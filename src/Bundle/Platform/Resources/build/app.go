...
import (
	...
	"solidworx.co/platform/app/internal/serverconfig"
...
// Identity of the application being served. Every value here is supplied at link time by the
// xcaddy shim (`go build -ldflags -X`), so this file stays generic and compiles on its own.
var (
...
// env reads one of the application's own environment variables, e.g. env("PORT") reads
// ACME_PORT when the binary was built with an env prefix of ACME.
...
func initializeApp() error {
	...
	envVars := map[string]string{
		envPrefix + "_CONFIG_DIR": filepath.Join(configDir, appName),
		envPrefix + "_ENV":        "prod",
		envPrefix + "_DEBUG":      "0",
		envPrefix + "_RUNTIME":    "frankenphp",
		"APP_PATH":                appPath,
	}
...
func setupCommands() {
	...
	runCmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if logFormat != "" {
				must(os.Setenv(envPrefix+"_LOG_FORMAT", logFormat))
			}
...
`
			...
			app.OnEvent = func(ctx context.Context, event lu.Event) {
				switch event.Type {
				case lu.AppStartup:
					for _, command := range strings.Split(bootCommands, ",") {
						command = strings.TrimSpace(command)
						if command == "" {
							continue
						}
						if err := runConsoleCommand(command); err != nil {
							log.Error(ctx, errors.Join(fmt.Errorf("failed to run boot command %q", command), err))
						}
					}
				case lu.AppRunning:
					if !skipIntro {
	...
	runCmd.PreRunE = func(cmd *cobra.Command, args []string) error {
		...
		if !cmd.Flags().Changed("enable-metrics") {
			v := env("ENABLE_METRICS")
			if v == "1" || strings.EqualFold(v, "true") {
				enableMetrics = true
			}
		}
		if !cmd.Flags().Changed("metrics-port") {
			if v := env("METRICS_PORT"); v != "" {
				if n, err := strconv.Atoi(v); err == nil {
					metricsPort = n
				}
			}
...
// isAppInstalled reports whether the embedded application considers itself installed. With no
// install-check command configured there is nothing to wait for, so workers start immediately.
func isAppInstalled() bool {
	if installCheckCommand == "" {
		return true
	}

	return runConsoleCommand(installCheckCommand) == nil
}
